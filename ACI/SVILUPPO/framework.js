let contactSearchCallback, platformClient, authToken, userId, employeeId, interactionId, pollingInterval, officialName,
	...
	organizationIdExternalContact = {
		"2f7fffb4-d815-4312-8b37-e992272a63a8" : "27e7187c-1456-4b66-8ea5-905613edab5b", 
		"38a3b0b6-19bb-4d2f-b2d5-73108fe49a1d" : "27e7187c-1456-4b66-8ea5-905613edab5b"
	},
	...
	allowedDivisionsExternalContact = [
		"2f7fffb4-d815-4312-8b37-e992272a63a8" , "38a3b0b6-19bb-4d2f-b2d5-73108fe49a1d"
	],
	...
	org = 'aci',
	version = `1.0.8`,
	queueTenantDataTableId = '038851b7-2cfb-4f81-a5d2-7d3d1969ee13',
	divisions = {},
	canMakeOutboundCalls = true,
	outboundDataTableId = '',
	groupNoOutbound = '',
	groupAddOutboundButton = '',
	canUseCustomOutboundButton = false,
	disableHistoryButtons = false;
    serviceNamesToBeRemovedTable = '',
	serviceNamesToBeRemoved = new Set(),
	audioCtx = new (window.AudioContext || window.webkitAudioContext || window.audioContext),
	FourProodosLoginUrl = 'https://apiaci.4proodos.net/public/login2',
	FourProodosQueueUrl = 'https://apiaci.4proodos.net/protected/getQueues';
	FourProodosCallDetailsUrl = 'https://apiaci.4proodos.net/protected/getDettaglioChiamate';
	FourProodosCallDetailsAniUrl = 'https://apiaci.4proodos.net/protected/getDettaglioChiamateNumeroChiamante';
...
function getUserId() {
	...
	return apiInstance
		.getUsersMe({ expand: ['groups', 'employerInfo', 'profileSkills', 'skills'] })
		.then(async (data) => {
			...
			skills = data.skills;
			profileSkills = data.profileSkills[0];
			console.log ('skill e profile' , skills, profileSkills)
		//	getDefaultRoutingId();
...
let apiInstance = new platformClient.ConversationsApi();
	return apiInstance.getConversationsCalls()
  .then((data) => {
    console.log(`getConversationsCalls success! data: ${JSON.stringify(data, null, 2)}`);
	const conversationInfo = [];
	
	// Itera su ogni entità nella risposta
	data.entities.forEach(entity => {
	  const conversationId = entity.id;
	  const startDate = entity.participants[0].startTime; // Assumendo che la data di inizio sia la stessa per tutti i partecipanti
  
	  // Aggiungi ID della conversazione e data di inizio all'array
	  conversationInfo.push({ conversationId, startDate });
	});
	
	const conversationIds = conversationInfo.map(conversation => conversation.conversationId);
...
styleRecordings.innerHTML = `
  ...
  #queueReport td {
	...
	#cronologyReportContainer {
		overflow:scroll
	}
  
	#cronologyReport table {
		width: 100%;
		border-collapse: collapse;
		margin-top: 20px; 
	}
	
	#cronologyReport th, #cronologyReport td {
		border: 1px solid #ddd;
		padding: 8px;
		text-align: left;
	}
	
	#cronologyReport thead {
...
max-width: 9999px;
  ...
  .modal-header-btn-cron > i {
    color:white!important;
    margin: 5px;
  }

  .modal-header-btn-cron {
    background: white;
    border: 0;
    padding: 4px;
    float: right;
  ...
  .modal-header-btn-cron.modal-close {
      background: white;
  }
...
function toggleCronologiaReport()  {
	let wrapper = $('.cronology-report-wrapper')
	wrapper.toggleClass('hidden');
	if (wrapper.is(':visible')) {
		getCronologyReport()
	}
...
function getQueueReport() {
	...
	if (!canSeeQueue) return
	let body = {
		username: 'purecloud@technevalue.com',
		password: '4proodosAPI'
	}
...
function getCronologyReport() {
	...
	postData(FourProodosLoginUrl, body).then(data => {
		if (!canSeeQueue) return
		fetch(FourProodosCallDetailsUrl + '?tenant=' + queueTenant + '&startTime=' + currentDate, {
			crossDomain: true, // no-referrer, *no-referrer-when-downgrade, origin, origin-when-cross-origin, same-origin, strict-origin, strict-origin-when-cross-origin, unsafe-url
			'Accept': 'text/csv',
			'Content-Type': 'application/csv',
			headers: {
			...
			.then(data => {
                createTableCron(data.trim(),currentPage, rowsPerPage)
			});
	})
	console.log('riga 941', 'arrivo qui')
}
...
function getCronologyReportFilter() {
	...
	console.log(currentDate,'currentDateFilter')
	
	let aniInput = $('#aniInput').val().trim();
	console.log('aniInput dentro filter', aniInput)
	...
	postData(FourProodosLoginUrl, body).then(data => {
		if (!canSeeQueue) return
		fetch(FourProodosCallDetailsAniUrl + '?tenant=' + queueTenant + '&startTime=' + currentDate + '&numeroChiamante=' + aniInput , {
			crossDomain: true, // no-referrer, *no-referrer-when-downgrade, origin, origin-when-cross-origin, same-origin, strict-origin, strict-origin-when-cross-origin, unsafe-url
			'Accept': 'text/csv',
			'Content-Type': 'application/csv',
			headers: {
			...
			.then(data => {
                createTableFilterCron(data.trim(),currentPage, rowsPerPage)
			});
	})
	console.log('riga 941', 'arrivo qui')
}
...
function changeDate(direction, event) {
    setInitialCurrentDate();


    let dateObject = new Date(currentDate);

    // Assume you have a variable tracking the date step, let's call it dateStep
    let dateStep = 30;

    if (direction === 'back') {
        dateObject.setDate(dateObject.getDate() - dateStep);
    ...
    // Call the function to update the table with the new date
    // For example, if the function that fetches data and updates the table is getCronologyReportFilter
    getCronologyReportFilter(currentDate);


}


function createTableCron(array, currentPage, rowsPerPage) {
    setInitialCurrentDate();

    let content = '';

    let currentArray = Array.isArray(array) ? array : array.split('\n').map(row => row.split(','));
    let [title, ...rest] = currentArray;
    let header = title;



    // Calculate start index based on currentPage and rowsPerPage
    let startIndex = (currentPage - 1) * rowsPerPage;

    let rowsToDisplay = rest.slice(startIndex, startIndex + rowsPerPage);
    ...
    if (!content.length) {
        content = '<tr><td colspan="' + header.length + '">No data</td></tr>';
    }
    ...
    for (let i = 1; i <= Math.ceil(rest.length / rowsPerPage); i++) {
        content += '<button onclick="changePage(' + i + ')">' + i + '</button>';
    }
    content += '</div>';

    // Set the content to the element with the id 'cronologyReport'
    $('#cronologyReport').html(content);
}

// Function to change the current page
function changePage(newPage) {
    currentPage = newPage;
    getCronologyReport(currentPage, rowsPerPage);
}

function createTableFilterCron(array, currentPage, rowsPerPage) {
    setInitialCurrentDate();
    ...
    content += '<div class="pagination">';
    for (let i = 1; i <= Math.ceil(rest.length / rowsPerPage); i++) {
        content += '<button onclick="changePageFilter(' + i + ')">' + i + '</button>';
    }
    content += '</div>';
...
// Function to change the current page
function changePageFilter(newPage) {
    currentPage = newPage;
    getCronologyReportFilter(currentPage, rowsPerPage);
}
...
function getDefaultRoutingId(pageNumber)
{
	let apiInstance = new platformClient.RoutingApi();
	console.log('profileskillsname', profileSkills)
	let opts = {
		name: profileSkills,
	...
	console.log('opts routingskill', opts)
  
	apiInstance
		.getRoutingSkills(opts)
		.then((data) => {
			console.log(data)
			skillsRoutingId = data.entities[0].id
			console.log('skillroutingid', skillsRoutingId)
			putDefaultSkill();
		})
		.catch((err) => {
...
function putDefaultSkill()
{
console.log('default skill dentro', skillsRoutingId)
let apiInstance = new platformClient.UsersApi();
let body = [{
	id: skillsRoutingId, 
	"proficiency": 1
...
function searchExternalContactOutbound(interaction, count) {
	if (!canSeeExternalContact) return
	if (!count) count = 1
	if (count > 10) return;
	let id = interaction.id
	...
	if (!interactionDiv) {
		return setTimeout(() => searchExternalContactOutbound(interaction, (count + 1)), 500)
	}
	getExternalcontactsContacts(interaction.calledNumber).then(data => {
		if (data.entities.length) {
			let contact = data.entities[0]
			let fullname = [contact.firstName, contact.middleName, contact.lastName].join(' ')
			//$('.string-added-to-interaction').remove()
			...
			interactionDiv.find('span.call-state>span').first().after(`<br><span class="string-added-to-interaction">
				<button onclick="addInteractionNumberToExternalContact('${interaction.calledNumber}')">
				Aggiungi A Rubrica <i class="fa fa-user-plus"></i>
			</span>`)
		}
	})
...
function startingSetup() {
	...
	//$('li[data-call-control="record"]').html(`
	//	<i class="fa fa-circle fa-lg" style="font-size: 23px;padding-top: 11px;color:white;cursor:pointer" tooltip-placement="bottom" tooltip-append-to-body="true"></i>
	//`); commentate queste 3 righe per evitare che appaia il bottone di recording
	$(
		'.interaction-call-control-container > .call-control-list > [data-call-control="record"]'
	).after(`
						<li style="order: 11; display: none;" data-call-control="flag" onclick="toggleRecentRecordings()">
			...
			<li style="order: 14; display: block" data-call-control="flag" class="cronIcon" onclick="toggleCronologiaReport()">
			<i class="fa fa-history" uib-tooltip="Cronology Report" style="font-size: 23px;padding-top: 11px;color:white;cursor:pointer"
			  tooltip-placement="bottom" tooltip-append-to-body="true"></i>
		  </li>
...
<!--                </div>-->
	  ...
	  <div class="cronology-report-wrapper hidden" id="draggableList5">
	  <div class="header" id="draggableListHeader">
		  <div class="list-header-recording">
		  <div>
           <div  class="modal-header-btn-cron"> 
		   <button onclick="getCronologyReportFilter()">Cerca il chiamante:</button><input type="text" id="aniInput">
		   <button onclick="changeDate(\'back\',event)">◄ Indietro di 30 giorni</button>
		   <button onclick="changeDate(\'forward\',event)">Avanti di 30 giorni ►</button>
               </div>
			 </div>
			<div onclick="toggleCronologiaReport()" class="modal-header-btn modal-close">
			  <i class="fa fa-close fa-2x"></i>
			</div>
		</div>
	  </div>
	  <div id="cronologyReportContainer">
		  <table id="cronologyReport"></table>
</div>
  </div>
					...
					<div class="deleteModalConfirm ml-8" hidden>
						...
						let contactId = $('#contactId')
						 $('#addContactForm').submit(e => {
							 ...
							 if (!tel.startsWith('+39') && tel.indexOf('+') !== 0) {
								tel = '+39' + tel.replace('+', '');
							}
						 ...
						 function addContact(contact) {
							...
							function paginate(pos) {
								let filter = $('#contactSearchInput').val()
								let pageNumber = parseInt($('#contactsPageNumber').val())
								let pageCount = parseInt($('#contactsPageCount').val())
								...
								if(pos === 'prev' && pageNumber > 1) {
									getExternalcontactsContacts(filter, pageNumber - 1)
									.then(data => renderExternalContact(data))
								}
...
function startSecureIvrSessionSofia(flowId, conversationId, timing = undefined) {
	console.log('entroinaggreeting', conversationId)
	let participantId
	let api = new platformClient.ConversationsApi();
	return api.getConversation(conversationId).then(data => {
		data.participants.forEach(p => {
			if (p.purpose === 'customer' && p.name != 'Extension') participantId = p.id;
		})
		let apiInstance = new platformClient.ConversationsApi();
		let opts = {
			body: {
...
function getExternalcontactsContacts(filter, page) {
	...
	let opts = {
		...
		'sortOrder': "firstName", // String | Sort order
...
window.Framework = {
	config: {
		...
		clientIds: {
			'mypurecloud.com': '',
			'mypurecloud.ie': '10177c30-fe13-40c6-beea-3fe3434d4c4f',
			'mypurecloud.com.au': '',
			'mypurecloud.jp': '',
			'mypurecloud.de': '' 
		},
		...
		settings: {
			...
			// searchTargets: ['externalContacts'],
			theme: {
				// primary: "#f82", // local
	...
	initialSetup: function () {
		...
		window.fetch(onHoldMusicUrl, {
			...
			mode: 'cors',
			headers: {
				'Access-Control-Allow-Origin': 'https://apps.mypurecloud.ie'
			}
		})
		...
		window.PureCloud.subscribe([
			{
				...
				callback: function (category, interaction) {
					...
					$('[data-call-control="securePause"]')[0].style.setProperty('display', 'none', 'important');
					...
					//	$('gef-dialpad-control').prop('disabled', true);
						...
						//$('gef-transfer-control').css('display','none'); per abilitare  l enable controls scommentare questo e quello sotto 
						/* if (transferEnable) {
				...
				//	console.log('conversationInfo', conversationInfo)
				    ...
				    if (category === 'add' & interaction.direction === 'Outbound' & interaction.ani === 'Interno') {
					$(
						'.interaction-call-control-container > .call-control-list > [data-call-control="record"]'
					).after(`
										<li style="order: 11; display: none;" data-call-control="flag" onclick="toggleRecentRecordings()">
					...
					//	$('gef-dialpad-control').prop('disabled', true);
						$('gef-secure-pause-control').prop('disabled', true);
						$('gef-record-control').prop('disabled', true);
						//$('gef-transfer-control').prop('disabled', true); per usare enablecontrols scommentare
						$('.string-added-to-interaction').remove()
					...
					if (category === 'change' && interaction.new.calledNumber && interaction.new.direction === 'Outbound' && interaction.new.state === 'COMPOSIZIONE IN CORSO'
					&& interaction.old.state === 'COMPOSIZIONE IN CORSO' && interaction.new.remoteName && interaction.old.phone === '') {
						if (!canSeeExternalContact) return
						searchExternalContactOutbound(interaction.new)
					}
					...
					if (category === 'connect' && interaction.isConnected) {
						...
						$('gef-secure-pause-control').prop('disabled', true);
						$('gef-record-control').prop('disabled', true);
					//	$('gef-transfer-control').prop('disabled', true); per disabilitare SEMPRE il transfer e usare l enablecontrols
						...
						if (interaction.attributes?.agentgreeting && interaction.attributes.agentgreeting.length > 10 
							 && (interaction.attributes?.sofia_check_ok_ko ?? []).length === 0) {
							let id = interaction.id
							getParticipant(id, 'customer').then(customer => {
						...
						if (interaction.attributes?.agentgreeting && interaction.attributes.agentgreeting.length > 10 
							&& interaction.attributes?.sofia_check_ok_ko.length > 1) {
						   let id = interaction.id
						   getParticipant(id, 'external').then(customer => {
							   let newAttrObjAgGreet = { agentgreetingtotrasf: interaction.attributes.agentgreeting }
							   console.log('newAttrObjAgGreet', newAttrObjAgGreet)
							   patchConversationParticipantAttributes(id, customer.id, newAttrObjAgGreet)

						   })
						   startSecureIvrSessionSofia(interaction.attributes.agentgreeting, interaction.id)
					   }
						...
						if (interaction.attributes?.agentgreetingtotrasf && interaction.attributes.custom_transferred === 'true'
							...
							startSecureIvrSessionSofia(interaction.attributes.agentgreetingtotrasf, interaction.id)
						}
					...
					//	enableRec(interaction.id);
						if (interaction.attributes && interaction.attributes.open_interaction) {
							...
							let ad_telnrlg_sms_c = interaction.attributes.ad_telnrlg_sms__c;
							let idchiamata_sfi = interaction.attributes.sfi_idchiamata__c;
							...
							try {
								...
								'telnrlg' + ad_telnrlg_sms_c, 'userid' + userId, 'ca' + contractaccountid)
						}
					...
					// 	console.log('primoif')
					// 	$('[data-call-control="securePause"]')[0].style.setProperty('display', 'none', 'important');
					// 	$(`[data-call-control="transfer"]`).hide();
					// 	$(`[data-call-control="scheduleCallback"]`).hide();
					// 	//$(`[data-call-control="dtmf"]`).hide();
		...
		window.addEventListener('click', function (evt) {
			...
			if (evt.target.textContent.includes('Pianificazione agente') || evt.target.textContent.includes('Interazioni')
				...
				|| evt.target.outerHTML.includes("<i class=\"fa fa-stack interactionCount\">")
				|| evt.target.outerHTML.includes("<i class=\"fa fa-circle fa-stack-2x interactionCount-badge\">")
				|| evt.target.outerHTML.includes("<i class=\"fa fa-inverse fa-stack-1x interactionCount-number\">")
